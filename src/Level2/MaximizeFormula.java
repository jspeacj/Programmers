...
public class MaximizeFormula {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < chars.length; i++) {
            if ((int) chars[i] >= 48 && (int)chars[i] <= 57) str += chars[i];
            else {
                expressSplitList.add(str);
                expressSplitList.add(String.valueOf(chars[i]));
                str = "";
            }

            if (i == chars.length - 1) expressSplitList.add(str);
        }
