...
public class Compression {
    public static void main(String[] args) {
        ...
        /* TC 1 answer : [11, 1, 27, 15] */
        //String msg = "KAKAO";

        /* TC 2 answer : [20, 15, 2, 5, 15, 18, 14, 15, 20, 27, 29, 31, 36, 30, 32, 34] */
        //String msg = "TOBEORNOTTOBEORTOBEORNOT";

        /* TC 3 answer : [1, 2, 27, 29, 28, 31, 30] */
        String msg = "ABABABABABABABAB";

        String[] strArrays = msg.split("");
        Map<String, Integer> map = new HashMap<>();
        List<Integer> list = new ArrayList<>();
        Queue<String> queue = new LinkedList<>();
        String str = "";
        String nextStr = "";
        int lastindex = 27; //Z다음 색인 번호

        // A ~ Z 색인 번호 부여
        for (int i = 0; i <= 25; i++) map.put(Character.toChars(i+65)[0] + "", i + 1);
        for (String s : strArrays) queue.add(s);
        ...
        while (iterator.hasNext()) {
            ...
            nextStr = str + queue.peek();
            if (!map.containsKey(nextStr)) {
                map.put(nextStr, lastindex++);
                list.add(map.get(str));
                str = "";
                nextStr  = "";
                continue;
        ...
        for (int index = 0; index < list.size(); index++) answer[index] = list.get(index);
