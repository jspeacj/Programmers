...
public class RotateMatrixBorders {
    ...
    private static int rotate(int[][] arrays, int[] arr, Queue<Integer> queue) {
        int min = Integer.MAX_VALUE;
        int minRow = arr[0] - 1;
        int maxRow = arr[2] - 1;
        int minCol = arr[1] - 1;
        int maxCol = arr[3] - 1;
        min = rotateStepOne(arrays, minRow, minCol, maxCol, min, queue);

        min = rotateStepTwo(arrays, maxCol, minRow, maxRow, min, queue);

        min = rotateStepThree(arrays, maxRow, minCol, maxCol, min, queue);

        min = rotateStepFour(arrays, minCol, minRow, maxRow, min, queue);

        return min;
    }

    private static int rotateStepOne(int[][] arrays, int row, int minCol, int maxCol, int min, Queue<Integer> queue) {
        for (int i = minCol; i <= maxCol; i++) {
            min = Math.min(min, arrays[row][i]);
            queue.add(arrays[row][i]);
    ...
    private static int rotateStepThree(int[][] arrays, int row, int minCol, int maxCol, int min, Queue<Integer> queue) {
        for (int i = maxCol; i > minCol; i--) {
            min = Math.min(min, Math.min(arrays[row][i - 1], arrays[row][i]));
    ...
    private static int rotateStepFour(int[][] arrays, int col, int minRow, int maxRow, int min, Queue<Integer> queue) {
        for (int i = maxRow; i > minRow; i--) {
            min = Math.min(min, Math.min(arrays[i-1][col], arrays[i][col]));
            queue.add(arrays[i - 1][col]);
            arrays[i - 1][col] = queue.poll();
        }
