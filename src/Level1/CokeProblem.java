...
public class CokeProblem {
    ...
    public static void main(String[] args) {
        ...
        int b = 1; // 빈 병 a개를 가져다 주면 마트가 주는 콜라 병 수
    ...
    public static void exchangeCoke(int a, int b, int n) {
        int c = (n / a) * b;

        if (c >= 1) answer += c;
        else return; // 더이상 콜라를 교환 할 수 없을 경우 종료

        n = n % a + c; // 교환하고 남은 빈 병 + 교환해서 얻은 콜라 병

        if (n >= a) exchangeCoke(a, b, n); // 빈병을 다시 만들 수 있을 경우 재귀 호출 이용하여 더 이상 교환 할 수 없을 때 까지 반복 수행
    }
}
