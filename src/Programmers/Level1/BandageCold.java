...
public class BandageCold {
    public static void main(String[] args) {
        ...
        int maxHalth = health; // 최대 체력
        int continueTime = 0; // 연속 성공
        int now = 0;
        int maxTime = attacks[attacks.length - 1][0]; // 오름차순 정렬이므로 가장 마지막 인덱스의 첫번쨰값이 마지막 공격시간대이다.
        int index = 0; // attacks의 공격시간대의 위치. 첫번째부터 시작
        
        while (now < maxTime) {
            now++;
            if (attacks[index][0] == now) {
                continueTime = 0;
                ...
                index++; //공격을 시도했으므로 다음 공격시간대로 이동
            } else {
                continueTime++;
                if (continueTime == bandage[0]) { // 정해진 시전 시간까지 치료에 성공할 경우 추가 회복 적용
                    continueTime = 0;
                    int addHealth = health += bandage[2];
                    health = addHealth > maxHalth ? maxHalth : addHealth;
                }
                int plusHealth = health += bandage[1];
                health = plusHealth > maxHalth ? maxHalth : plusHealth;
            }
        }
