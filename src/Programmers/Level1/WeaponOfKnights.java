...
public class WeaponOfKnights {
    public static void main(String[] args) {
        ...
        int limit = 3; //제한 수치
        int power = 2; //제한 수치를 초과한 기사가 사용할 무기의 공격력*/
        ...
        for (int index = 1; index <= number; index++) {
            int measure = getMeasure(index);
            if (measure > limit) measure = power;
    ...
    public static int getMeasure(int index) {
        int measure = 0;
        for (int i = 1; i <= (int)Math.sqrt(index); i++) {
            if (index % i == 0) {
                if (index / i == i) measure++; //제곱근일 경우 가운데 값이므로 하나만 더한다.
                else measure += 2; // 제곱근이 아닐 경우 제곱근 기준 왼쪽 오른쪽 대칭 개수를 더하므로 2개를 더한다.
            }
        }
