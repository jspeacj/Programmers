...
public class NeighboringSquare {
    public static void main(String[] args) {
        ...
        System.out.println(getSameColorCnt(board, h, w));
    }

    public static int getSameColorCnt (String[][] board, int h, int w) {
        int cnt = 0;
        String color = board[h][w];

        if (h > 0) if (color.equals(board[h - 1][w])) cnt++;
        if (h < board.length - 1) if (color.equals(board[h + 1][w])) cnt++;
        if (w > 0) if (color.equals(board[h][w - 1])) cnt++;
        if (w < board[h].length - 1) if(color.equals(board[h][w + 1])) cnt++;
