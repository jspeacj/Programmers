...
public class BanknoteFolding {
    public static void main(String[] args) {
        ...
        int cnt = 0;
        while (true) {
            if (chkLength(wallet, bill)) break;
    ...
    public static boolean chkLength(int[] wallet, int[] bill) {
        boolean foldFlag = false;
        if (bill[0] <= wallet[0] && bill[1] <= wallet[1]) foldFlag = true;
        if (bill[1] <= wallet[0] && bill[0] <= wallet[1]) foldFlag = true;
        return foldFlag;
    }
}
