...
public class PersonalityTypeTest {
    public static void main(String[] args) {
        ...
        * */
        ...
        String[] type = {"RT", "CF", "JM", "AN"}; // 성격 유형 모음
        
        for (int i = 0; i < survey.length; i++) {
            sb.setLength(0); // 초기화
            int choice = choices[i] - 4;

            if (choice > 0) sb.append(survey[i].substring(1,2)); // 동의 관련 선택지 선택
            else if (choice < 0) sb.append(survey[i].substring(0,1)); // 비동의 관련 선택지 선택

            map.put(sb.toString(), map.getOrDefault(sb.toString(), 0) + Math.abs(choice));
        }


        for (String str : type) {
            String left = str.substring(0, 1);
            String right = str.substring(1, 2);

            if (map.getOrDefault(left,0) > map.getOrDefault(right,0)) result.append(left);
            else if (map.getOrDefault(left,0) < map.getOrDefault(right, 0)) result.append(right);
            else {
                // 둘다 점수가 없을 경우(0), 사전 순으로 빠른 유형으로 정한다.
                if ((int)left.charAt(0) > (int)right.charAt(0)) result.append(right); // 오른쪽 타입이 사전순으로 더 빠를 경우
                else result.append(left); // 왼쪽 타입이 사전순으로 더 빠를 경우
            }
        }
