...
public class ExpectedBracket {
    public static void main(String[] args) {
        ...
        while (true) {
            double nextA = Math.ceil((double) a / 2);
            double nextB = Math.ceil((double) b / 2);
            answer++;

            double max = Math.max(nextA, nextB);

            if (nextA == nextB) {
                answer--;
                break;
            } else if (max % 2 == 0) {
                if (max == nextA && nextA - 1 == nextB) break;
                if (max == nextB && nextB - 1 == nextA) break;
            }
