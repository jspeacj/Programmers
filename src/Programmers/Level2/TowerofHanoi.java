...
public class TowerofHanoi {
    ...
    public static void main(String[] args) {
        ...
        /* TC 1 result : [[1,2], [1,3], [2,3]] */
        int n = 2;

        /*
            규칙
            1. 1번 기둥의 n-1개를 3번을 걸쳐 2번으로 이동
            2. 가장 큰 n을 1에서 3으로 이동
            3. 2번 기둥의 n-1개를 1번을 걸쳐 3번으로 이동
        * */

        dfs(n, 1, 3, 2);
        int[][] answer = new int[list.size()][2];
        for (int index = 0; index < answer.length; index++) answer[index] = list.get(index);
    ...
    private static void dfs(int n, int start, int to, int mid) {
        ...
        // 1번 기둥의 n-1개를 3번을 걸쳐 2번으로 이동시킨다.
        dfs(n-1, start, mid, to);

        // 가장 큰 n을 1에서 3으로 이동시킴
        list.add(new int[]{start, to});

        // 2번 기둥의 n-1개를 1번을 걸쳐 3번으로 이동시킴
