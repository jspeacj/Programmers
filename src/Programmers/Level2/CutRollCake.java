...
public class CutRollCake {
    public static void main(String[] args) {
        /*
            ...
            입출력 예 설명
            입출력 예 #1
            롤케이크를 [1, 2, 1, 3], [1, 4, 1, 2] 또는 [1, 2, 1, 3, 1], [4, 1, 2]와 같이 자르면
            철수와 동생은 각각 세 가지 토핑을 맛볼 수 있습니다.
        ...
        int answer = 0;
        int index = 0;
        Set<Integer> culsu = new HashSet<>();
        Set<Integer> brother = new HashSet<>();

        int length = topping.length + 1;
        for (int num : topping) if (num > length) length = num + 1;
        ...
        while (true) {
            if (culsu.size() == brother.size()) answer++;
            else if (culsu.size() > brother.size()) break;
            int num = topping[index++];

            if (duplicate[num] > 1) duplicate[num]--;
            else brother.remove(num);
