...
public class CheckYourDistance {
    public static void main(String[] args) {
         ...
         */
        /* TC 1 result : [1, 0, 1, 1, 1] */
        //String[][] places = {{"POOOP", "OXXOX", "OPXPX", "OOXOX", "POXXP"}, {"POOPX", "OXPXP", "PXXXO", "OXXXO", "OOOPP"}, {"PXOPX", "OXOXP", "OXPOX", "OXXOP", "PXPOX"}, {"OOOXX", "XOOOX", "OOOXX", "OXOOX", "OOOOO"}, {"PXPXP", "XPXPX", "PXPXP", "XPXPX", "PXPXP"}};
        String[][] places = {{"POOOP", "OXXOX", "OXXPX", "OPXOX", "PXXXP"}, {"POOPX", "OXPXP", "PXXXO", "OXXXO", "OOOPP"}, {"PXOPX", "OXOXP", "OXPOX", "OXXOP", "PXPOX"}, {"OOOXX", "XOOOX", "OOOXX", "OXOOX", "OOOOO"}, {"PXPXP", "XPXPX", "PXPXP", "XPXPX", "PXPXP"}};

        int[] answer = new int[5];

        for (int j = 0; j < places[0].length; j++) {
            for (char c : places[0][j].toCharArray()) {
                System.out.print(" " + c + " ");
            }
            System.out.println();
        }
    ...
    private static boolean check(String[][] places, int index) {
        boolean flag = true;
        String[][] arr = new String[5][5];

        for (int i = 0; i < places[index].length; i++) {
            int next = 0;
            for (char c : places[index][i].toCharArray()) arr[i][next++] = String.valueOf(c);
        }
    ...
    private static boolean checkDistanc(String[][] places, int i, int j) {
        ...
        if (i + 1 < 5 && j + 1 < 5 && (!"X".equals(places[i+1][j]) || !"X".equals(places[i][j+1]))) {
            if ("P".equals(places[i+1][j+1])) return false;
        }
