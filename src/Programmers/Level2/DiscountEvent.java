...
public class DiscountEvent {
    public static void main(String[] args) {
        ...
        for (String s : want) wantList.add(s);

        for (int i = 0; i < discount.length; i++) {
            boolean flag = true;
            init(countList, number);
            for (int j = i; j < discount.length; j++) {
                int index = wantList.indexOf(discount[j]);
                if (index == -1) break;
                else {
                    if (countList.get(index) - 1 < 0) break;
                    else countList.set(index, countList.get(index) - 1);
                }
            }
    ...
    public static void init(List<Integer> countList, int[] number) {
        countList.clear();
        for (int i : number) countList.add(i);
    }
}
